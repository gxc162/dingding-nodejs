 ...
 * @LastEditTime: 2020-10-10 10:32:53
...
let saveOrUpdate = (param,handle)=>{
    pool.getConnection((err,conn)=>{
        ...
        if(param.id){
            //更新
            sql = 'update ej_category set name=?,num=?,icon=?,parent_id=? where id=?';
        }else{
            //新增
            sql = 'insert into ej_category(name,num,icon,parent_id) values(?,?,?,?)';
        }
        conn.query(sql,[param.name,param.num,param.icon,param.parent_id,param.id],(err,results)=>{
            if(err) throw err;
            handle(results);
            conn.release();
        })
...
let batchDelete = (param,handle)=>{
    pool.getConnection((err,conn)=>{
        ...
        let sql = 'delete from ej_category where id in (?)';
...
let pageQuery = (param,handle)=>{
    pool.getConnection((err,conn)=>{
        if(err)throw err;
        let sql = 'select * from ej_category limit ?,?';
...
let mohuQuery = (param,handle)=>{
    pool.getConnection((err,conn)=>{
        if(err) throw err;
        let sql = 'select * from ej_category where name like ?'
